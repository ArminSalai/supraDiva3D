...
/*Show the cookie container or remove the cookie message before apperance, if cookies are already
accepted, and switch to light/dark mode based on previous choices*/
...
//Orbit controls and the contol buttons
...
//Resize renderer on load and on window size change
...
scene.add(hemiLight);
//Loading screen and page while the model is loading
var played = false;
...
loader.load("assets/models/DivaGrey.glb", function (glb) {
    ...
    scene.add(diva);
    window.scrollTo(0, 0);
    diva.matrixAutoUpdate = false;
    diva.rotation.x = Math.PI/2;
    diva.updateMatrix();
});
//Update function
function animate() {
